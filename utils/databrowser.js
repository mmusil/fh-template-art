  ...
  return new Promise(function(resolve, reject){
      request(options,
              function(err, res, body){
                  if (err || res.statusCode !== 200) return reject(err ? err: `${res.statusCode}: ${res.statusMessage}`);
    ...
    return new Promise(function(resolve, reject){
        request(options,
                function(err,res,body){
                    if (err || res.statusCode !== 200) return reject(err ? err: `${res.statusCode}: ${res.statusMessage}`);
                    body = JSON.parse(body);
                    var item = body.list.find((dbItem) => {
                        return dbItem.fields[Object.keys(dbItem.fields)[0]] === dbEntry 
                    });
